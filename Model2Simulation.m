...
% Total runtime for this code was about 1-3 minutes on the computer used by
...
% P_IT1         Probability of a healthy cell receiving therapy of becoming
%               infected
...
P_iT = 0.07;        % probability of infection of healthy with therapy 1
...
savesteps = [3 7 11 15 20 25 50 100 150 200 300 400 500];
%timesteps for which we want to save simulation image

%% States

% State 1: H:       Healthy                 (Color- Green)
% State 2: H_T:    Healthy with Therapy    (Color- Red)
...
set(figure, 'OuterPosition', [100 30 1000 740]) % sets figure window size
% outerposition values are [left, bottom, width, height]
print2(grid);   % prints the initial grid

%% Simulation

taugrid = zeros(n); % to initially set number of timesteps that a cell has
% been in state 2 to zero for every cell
state = zeros(5,totalsteps);    % initializes a grid to keep count of cells
% in every state
timestep = 1;
while timestep<=totalsteps
    ...
    for x=2:n-1
        for y=2:n-1
            ...
            % Rule 1
            % If the cell is in H state, it can become H_T or I_1
            if(grid(x,y)==1)
                
                % Rule 1. a  It can become H_T state with probability P_T1
                if(rand<=P_T && timestep >= T_start)
                    nextgrid(x,y) = 2;
                    continue;
            ...
            % Rule 2
            % If the cell is in state H_T, it can become I_1 or H
            if(grid(x,y) == 2)
                
                % Rule 2. a
                % It can become I_1 with probability P_IT1 if neighbor
                % condition is met
                if((rand <= P_iT) && (((rand <= P_i) && ...
                        (grid(x-1,y-1)==3 || grid(x-1,y)==3 || ...
                        grid(x-1,y+1)==3 || grid(x,y-1)==3 || ...
                        grid(x,y+1)==3 || grid(x+1,y-1)==3 || ...
                        grid(x+1,y)==3 || grid(x+1,y+1)==3 || ...
    ...
    print2(grid);       % to display the grid at the end of each timestep
    
    % to save the image of the simulation every 50 timesteps as a pdf
    if(find(savesteps==timestep))
        saveas(gcf,strcat('Model2Timestep',num2str(timestep),'.pdf'));
    end
    
    state(1,timestep) = sum(sum(grid(2:n-1,2:n-1)==1)); % H
    state(2,timestep) = sum(sum(grid(2:n-1,2:n-1)==2)); % H_T
    state(3,timestep) = sum(sum(grid(2:n-1,2:n-1)==3)); % I_1
    state(4,timestep) = sum(sum(grid(2:n-1,2:n-1)==4)); % I_2
    state(5,timestep) = sum(sum(grid(2:n-1,2:n-1)==5)); % D
...
set(figure, 'OuterPosition', [200 100 700 600]) % sets figure window size
plot( 1:totalsteps , state(1,:), 'g', ...
    1:totalsteps , state(2,:), 'r', ...
    1:totalsteps , state(3,:), 'c', ...
    1:totalsteps , state(4,:), 'b', ...
    1:totalsteps , state(5,:), 'k' , 'linewidth', 2 );
legend( 'Healthy', 'Healthy with Therapy', 'Acute Infected', 'Latent Infected', ...
    'Dead', 'Location' ,'NorthEast' );
gridxy(20,'Color',[0.8 0.5 0.0],'linewidth',5) ;
saveas(gcf,strcat('Model2GraphSingleRun.pdf'));
