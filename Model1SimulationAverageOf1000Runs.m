...
% Total runtime for this code was about 1 hour on the computer used by
...
% When run, this code does not display anything till the end of the 1 hour
...
for run = 1:totalruns
    ...
    state1(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==1));
    state2(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==2));
    state3(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==3));
    state4(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==4));
...
plot( 1:totalsteps , state1mean, 'g', ...
    1:totalsteps , state4mean, 'c', ...
    1:totalsteps , state3mean, 'b', ...
    1:totalsteps , state2mean, 'k' , 'linewidth', 2 );
xlim([0 600]); 
ylim([0 10000]);
hold on;
errorbar( 1:15:totalsteps , state1mean(1:15:totalsteps), state1dev(1:15:totalsteps), 'g');
errorbar( 1:15:totalsteps , state4mean(1:15:totalsteps), state4dev(1:15:totalsteps), 'c');
errorbar( 1:15:totalsteps , state3mean(1:15:totalsteps), state3dev(1:15:totalsteps), 'b');
errorbar( 1:15:totalsteps , state2mean(1:15:totalsteps), state2dev(1:15:totalsteps), 'k');
legend( 'Healthy', 'Acute Infected', 'Latent Infected', 'Dead', ...
    'Location' ,'NorthEast' );
saveas(gcf,strcat('Model1Graph1.pdf'));
hold off;
...
set(figure, 'OuterPosition', [200 100 700 500]) % sets figure window size
plot( 1:totalsteps , state1mean, 'g', ...
    1:totalsteps , state5mean, 'b', ...
    1:totalsteps , state2mean, 'k' , 'linewidth', 2 );
xlim([0 600]); 
ylim([0 10000]);
hold on;
errorbar( 1:15:totalsteps , state1mean(1:15:totalsteps), state1dev(1:15:totalsteps), 'g');
errorbar( 1:15:totalsteps , state5mean(1:15:totalsteps), state5dev(1:15:totalsteps), 'b');
errorbar( 1:15:totalsteps , state2mean(1:15:totalsteps), state2dev(1:15:totalsteps), 'k');
legend( 'Healthy', 'Infected', 'Dead', ...
    'Location' ,'NorthEast' );
saveas(gcf,strcat('Model1Graph2.pdf'));
