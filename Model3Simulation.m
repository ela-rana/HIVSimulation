...
print3(grid);   % prints the initial grid
saveas(gcf,strcat('Model3Timestep0.pdf'));

%% Simulation

taugrid = zeros(n); % to initially set number of timesteps that a cell has
% been in state 2 to zero for every cell
state = zeros(7,totalsteps);    % initializes a grid to keep count of cells
% in every state
timestep = 1;
while timestep<=totalsteps
        ...
        for y=2:n-1
            ...
            if(grid(x,y)==2)
                ...
                elseif((random1 <= P_iT1) && (((random1 <= P_i) && ...
                    ...
                    continue;
                % Rule 5.c
                % Otherwise, if there is probability of receiving therapy 2 
                % but not
                % therapy 1, then it becomes healthy with therapy 2
    ...
    print3(grid);       % to display the grid at the end of each timestep
    
    % to save the image of the simulation every 25 timesteps as a pdf
    if(find(savesteps==timestep))
        saveas(gcf,strcat('Model3Timestep',num2str(timestep),'.pdf'));
    end
    
    state(1,timestep) = sum(sum(grid(2:n-1,2:n-1)==1)); % H
    state(2,timestep) = sum(sum(grid(2:n-1,2:n-1)==2)); % H_T1
    state(3,timestep) = sum(sum(grid(2:n-1,2:n-1)==3)); % H_T2
    state(4,timestep) = sum(sum(grid(2:n-1,2:n-1)==4)); % H_Tb
    state(5,timestep) = sum(sum(grid(2:n-1,2:n-1)==5)); % I_1
    state(6,timestep) = sum(sum(grid(2:n-1,2:n-1)==6)); % I_2
    state(7,timestep) = sum(sum(grid(2:n-1,2:n-1)==7)); % D
...
plot( 1:totalsteps , state(1,:), 'g', ...
    ...
    1:totalsteps , state(3,:), 'm', ...
    1:totalsteps , state(4,:), 'y', ...
    1:totalsteps , state(5,:), 'c', ...
    1:totalsteps , state(6,:), 'b', ...
    1:totalsteps , state(7,:), 'k', 'linewidth', 2 );
...
saveas(gcf,strcat('Model3GraphSingleRun.pdf'));
