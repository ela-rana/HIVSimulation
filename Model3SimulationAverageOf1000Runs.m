...
% Total runtime for this code was about 1.5 hours on the computer used by
...
% When run, this code does not display anything till the end of the 1.5 
% hours.
...
for run = 1:totalruns
    ...
    while timestep<=totalsteps
        ...
        state2(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==2)); % H_T1
        state3(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==3)); % H_T2
        state4(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==4)); % H_Tb
        state5(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==5)); % I_1
        state6(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==6)); % I_2
        state7(run,timestep) = sum(sum(grid(2:n-1,2:n-1)==7)); % D
...
saveas(gcf,strcat('Model3Graph1.pdf'));
...
legend( 'Therapy start week', 'Healthy', 'Infected', 'Dead', ...
    'Location' ,'NorthEast' );
saveas(gcf,strcat('Model3Graph2.pdf'));
